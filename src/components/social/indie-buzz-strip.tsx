...
export function IndieBuzzStrip({ releases, news }: { releases: IndieRelease[]; news: IndieNewsItem[] }) {
  ...
  return (
    <div className="border-b border-border px-4 py-4 sm:px-5">
      <span className="text-[10px] uppercase tracking-wider text-foreground-muted">Indie Buzz</span>
        ...
        <div className="-mx-1 mt-3 flex gap-2 overflow-x-auto px-1 pb-1">
          {news.slice(0, 6).map((item, i) => (
            <a
              ...
              className="shrink-0 whitespace-nowrap rounded-full border border-border bg-surface px-3 py-1.5 text-xs text-foreground hover:border-accent/50 hover:text-accent"
            >
