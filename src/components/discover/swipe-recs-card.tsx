...
import { MatchToast, SwipeActionRow, SwipeDeckCaughtUp } from "./swipe-deck-parts";
...
export function SwipeRecsCard({ initialDeck }: { initialDeck: SwipeRec[] }) {
  ...
  if (!current) {
    return (
      <SwipeDeckCaughtUp
        ...
        onStartMovieNight={() => router.push("/movie-night")}
      />
    );
  }
  ...
  const matchToastEl = <MatchToast match={matchToast} />;
  ...
  const actionRow = <SwipeActionRow onPass={() => decide("left")} onSave={() => decide("right")} />;
