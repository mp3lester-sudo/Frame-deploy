...
export async function inviteToMovieNight(input: z.infer<typeof inviteSchema>) {
  ...
  if ((participantCount ?? 0) >= maxParticipants) {
    throw new Error(
      `This movie night is full (${maxParticipants} people max on your plan). Upgrade to Slate Auteur for bigger groups.`
    );
  }
 ...
 * works when the invitee is already a Slate account. resolve_movie_night_token
