...
describe("parseLetterboxdCsv", () => {
  it("parses a ratings.csv-shaped file, including quoted names with commas", () => {
    ...
    expect(rows).toEqual([
      { name: "Top Gun", year: 1986, rating: 5 },
      { name: "Paris, Texas", year: 1984, rating: 4.5 },
    ]);
  });
  ...
  it("parses a watched.csv-shaped file with no Rating column", () => {
    ...
    const rows = parseLetterboxdCsv(csv);
    expect(rows).toEqual([{ name: "Arrival", year: 2016, rating: null }]);
  });
  ...
  it("skips rows with no Name and tolerates a missing/malformed Year", () => {
    ...
    const rows = parseLetterboxdCsv(csv);
    expect(rows).toEqual([
      { name: "Some Film", year: null, rating: 3 },
      { name: "Another Film", year: 2021, rating: 2 },
    ]);
  });
});
...
describe("buildTitleIndex / matchTitle", () => {
  ...
  it("matches on exact name + year", () => {
    expect(matchTitle({ name: "Arrival", year: 2016, rating: 4 }, index)).toBe("1");
  });

  it("matches case-insensitively", () => {
    expect(matchTitle({ name: "arrival", year: 2016, rating: null }, index)).toBe("1");
  });

  it("falls back to an unambiguous name match within one year", () => {
    expect(matchTitle({ name: "Gravity", year: 2014, rating: 5 }, index)).toBe("4");
  });
  ...
  it("refuses to guess when the name matches multiple candidates ambiguously", () => {
    ...
    expect(matchTitle({ name: "Swan Song", year: 2005, rating: 3 }, index)).toBeNull();
  });

  it("resolves an ambiguous name when the year narrows it to one candidate", () => {
    expect(matchTitle({ name: "Swan Song", year: 2021, rating: 3 }, index)).toBe("2");
    expect(matchTitle({ name: "Swan Song", year: 1992, rating: 3 }, index)).toBe("3");
  });

  it("returns null for a title with no name match at all", () => {
    expect(matchTitle({ name: "Totally Made Up Film Title", year: 2020, rating: 3 }, index)).toBeNull();
  });

  it("matches an unambiguous single-candidate title even with no year in the CSV row", () => {
    expect(matchTitle({ name: "Arrival", year: null, rating: null }, index)).toBe("1");
  });
});
